...
const hexToRgb = (hex: string): [number, number, number] => {
  let cleanHex = hex?.trim() || '#d1651c';
  if (cleanHex.startsWith('var(') && typeof document !== 'undefined') {
    const varName = cleanHex.replace(/var\((--[^,\s)]+).*/, '$1');
  ...
  if (!result) return [0.82, 0.40, 0.11];
  return [parseInt(result[1], 16) / 255, parseInt(result[2], 16) / 255, parseInt(result[3], 16) / 255];
};

const colorModeToFloat = (mode: 'molten' | 'ember' | 'frost'): number => {
  return mode === 'ember' ? 1.0 : mode === 'frost' ? 2.0 : 0.0;
};
